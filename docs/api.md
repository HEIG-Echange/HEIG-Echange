...
- `201` → `{ id, ownerId, categoryId, title, description, itemCondition, location, status: "available", isPriority, endPriorityAt, priorityGroupIds?, photoCount: 0, shareUrl, qrUrl }`
...
- `200` → l'annonce mise à jour (même forme que `GET /listings/:id` sans le
  tableau `photos`)
- `400` - aucun champ fourni, champ invalide, `status` inconnu, `categoryId`
  inexistant, ou `priorityGroupIds` invalide
- `401` - pas connecté
...
- `200` → annonce (mêmes champs que `GET /listings`, + `location`,
  `ownerEmail`, `isPriority`, `endPriorityAt`) +
  `photos: [{ id, url, absoluteUrl, position }]`, ordonnées par `position`
  (la position 0 est la vignette). Si le visiteur connecté en est le
  propriétaire, inclut aussi `priorityGroupIds`.
- `400` - id non numérique
- `404` - annonce introuvable, propriétaire suspendu/bloqué, **ou annonce
  restreinte et inaccessible** au visiteur (voir encadré sous `POST /listings`)

# Documentation — HEIG Échange

Index de la documentation du projet.

| Document | Contenu |
|---|---|
| [description-projet.md](description-projet.md) | Problématique, solution, requirements fonctionnels et non-fonctionnels |
| [processus-travail.md](processus-travail.md) | Équipe, rôles, git flow, outils |
| [api.md](api.md) | Documentation de l'API REST (endpoints, auth, erreurs) |
| [frontend.md](frontend.md) | Application web : organisation, paliers responsive, densités d'affichage |
| [base-de-donnees.md](base-de-donnees.md) | Schéma MariaDB, accès admin, migrations |
| [deploiement.md](deploiement.md) | Pipeline CI/CD, environnements, secrets |
| [mockups/](mockups/) | Maquettes des écrans + landing page |

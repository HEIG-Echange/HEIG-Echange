...
DEFAULT_BASE_HOST = os.environ.get("PUBLIC_BASE_URL", "http://localhost")
DEFAULT_BASE_PORT = os.environ.get("APP_PORT", "3000")
DEFAULT_BASE_URL = os.environ.get("SEED_BASE_URL", f"{DEFAULT_BASE_HOST}:{DEFAULT_BASE_PORT}")
...
LISTINGS = [
    {
        ...
        "imageKey": "calculatrice-hp-prime-g2",
    ...
    {
        "category": "livres",
        "imageKey": "analyse-1-recueil-exercices",
    ...
    {
        "category": "sport",
        "imageKey": "raquette-badminton-yonex",
    ...
    {
        "category": "livres",
        "imageKey": "clean-code-robert-martin",
    ...
    {
        "category": "cuisine",
        "imageKey": "bouilloire-electrique",
    ...
    {
        "category": "livres",
        "imageKey": "physique-halliday-resnick",
    ...
    {
        "category": "livres",
        "imageKey": "introduction-bases-donnees",
    ...
    {
        "category": "mobilier",
        "imageKey": "matelas-simple-90x200",
    ...
    {
        "category": "vetements",
        "imageKey": "chaussures-securite-43",
    ...
    {
        "category": "electronique",
        "imageKey": "disque-dur-externe-500go",
...
def load_listing_photos(photos_dir: str | None, image_key: str, count: int) -> list[tuple[str, bytes]]:
    """Charge les photos correspondant exactement a une annonce.

    Convention :
      <imageKey>-1.jpg  -> photo principale
      <imageKey>-2.jpg  -> photo secondaire (optionnelle)
      <imageKey>-3.jpg  -> photo secondaire (optionnelle)

    Les extensions .jpeg, .png et .webp sont egalement acceptees.
    Aucune image d'une autre annonce n'est reutilisee.
    """
    if not photos_dir:
        raise SystemExit(
            "Aucun dossier d'images fourni. Utilisez --photos-dir ./sample-images"
        )

    if not os.path.isdir(photos_dir):
        raise SystemExit(f"Dossier d'images introuvable : {photos_dir}")

    files: list[tuple[str, bytes]] = []
    for number in range(1, count + 1):
        found = None
        for ext in IMAGE_EXTENSIONS:
            candidate = os.path.join(photos_dir, f"{image_key}-{number}{ext}")
            if os.path.isfile(candidate):
                found = candidate
                break

        if not found:
            # La photo principale est obligatoire. Les suivantes sont simplement
            # absentes si vous n'avez fourni qu'une seule photo pertinente.
            if number == 1:
                raise SystemExit(
                    f"Image principale manquante pour '{image_key}'.\n"
                    f"Attendu : {os.path.join(photos_dir, image_key + '-1.jpg')}"
                )
            break

        mime = mimetypes.guess_type(found)[0]
        if mime not in {"image/jpeg", "image/png", "image/webp"}:
            raise SystemExit(f"Format non supporte : {found}")

        with open(found, "rb") as handle:
            files.append((os.path.basename(found), handle.read()))

    return files


def validate_listing_images(photos_dir: str | None) -> None:
    """Verifie toutes les images avant de commencer les uploads."""
    missing = []
    for spec in LISTINGS:
        found = any(
            os.path.isfile(os.path.join(photos_dir or "", spec["imageKey"] + "-1" + ext))
            for ext in IMAGE_EXTENSIONS
        )
        if not found:
            missing.append(spec["imageKey"])

    if missing:
        raise SystemExit(
            "Images principales manquantes :\n  - "
            + "\n  - ".join(missing)
            + "\n\nChaque annonce doit au minimum avoir : <imageKey>-1.jpg"
        )
...
def photo_files(spec: dict, count: int, photos_dir: str) -> list[tuple[str, bytes]]:
    """Renvoie uniquement les photos pertinentes pour cette annonce."""
    return load_listing_photos(photos_dir, spec["imageKey"], count)
...
def main() -> int:
    ...
    parser.add_argument(
        "--photos-dir",
        help="Dossier contenant les images nommees <imageKey>-1.jpg, <imageKey>-2.jpg, etc. Obligatoire.",
    )
    parser.add_argument(
        "--reset",
        action="store_true",
    ...
    validate_listing_images(args.photos_dir)
    print(f"Images : {args.photos_dir} (association explicite par imageKey)")
    ...
    for spec in planned:
        ...
        files = [
            ...
            for name, content in photo_files(
                spec, spec["photos"], args.photos_dir
            )
        ]
